            ...
            <div class="card m-b-20">
                ...
                <form method="post" action="{{route('pengiriman.store')}}">
                    ...
                    <div class="form-row">
                        ...
                        <input type="text" hidden  id="tgl_jatuh_tempo" name="tgl_jatuh_tempo">
                        ...
                        <div class="form-group clearfix">
                            <div class="row">
                                <div class="col-md-6">
                                    ...
                                    <select name="jenis_pembayaran" id="jenis_pembayaran" class="form-control "></select>
                                </div>
                                <div class="col-md-6" >
                                    <label class="form-label" id="rekening_label" for="rekening">Rekening</label>
                                    <input type="text" name="rekening" id="rekening" class="form-control"style="display: none">
                                </div>
                            </div>
                        </div>
                        <div class="form-group clearfix">
...
{{--                        s--}}
                        ...
                        <div class="form-group ">
                            <div class="row">
                                <div class="col-md-8">
                                    <label style="float: right" class="form-label" id="bayarjual">Bayar</label>
                                </div>
                    ...
                    success: function (res) {
                        if (res) {
                            ...
                            $.each(res, function (i) {
                                ...
                                var anggo = res[i].pemesanan[0].penawaran;
                                console.log(anggo)
                                if (anggo != null){
                                    var anggota_html = 'Ya';
                                }else{
                                ...
                                console.log()
                                if (res[i].pemesanan[0].no_penawaran == null) {
                                    var no_penawaran = '';
                                }else{
                                    var no_penawaran = res[i].pemesanan[0].no_penawaran;
                                }
                                ...
                                // var pel = res[i].pemesanan[0].penawaran.anggota;

                               if (res[i].pemesanan[0].penawaran != 0){
                                    var pel1 = res[i].pemesanan[0].non_anggota
                                }else{
                                    var pel1 = res[i].pemesanan[0].non_anggota;
                                }

                                // id jenis_pembayaran append dropdown jenis_pembayaran
                                var rekening = res[i].pemesanan[0].rekenings;
                               console.log(rekening)
                                if (rekening == null) {
                                    $('#jenis_pembayaran').append('<option value="tunai" >Tunai</option>');
                                    $('#jenis_pembayaran').append('<option value="transfer" >Transfer</option>');
                                    $('#banks').hide(300);
                                    $('#rekening_label').hide(300);
                                    $('#bayar').show(300);
                                    $('#bayarjual').show(300);


                                }else{
                                    $('#jenis_pembayaran').append('<option value="rekening" >Rekening</option>');
                                    $('#jenis_pembayaran').append('<option value="tunai" >Tunai</option>');
                                    $('#jenis_pembayaran').append('<option value="transfer" >Transfer</option>');
                                    $('#rekening').val(rekening.saldo).show(400)
                                    $('#rekening_saldo').show(400)
                                    $('#banks').hide(300);
                                    $('#bayar').val('0').hide(300);
                                    $('#sisa_tagihan').val('0');
                                    $('#bayarjual').hide(300);
                                    //    selected #jenis_pembayaran == tunai
                                }
                                $('#id_bank').select2();
                                ...
                                // trigger #jenis_pembayaran change bank
                                ...
                                $('#pelanggan1').val(pel1)
                                console.log(res[i].pemesanan[0].pelanggan)
...
{{--                                var myOptions = '<option></option> @foreach ($pro1duk as $produks)<option data-harga="{{$produks->harga_jual}}"data-produk="{{$produks->kode_produk}}"data-satuan="{{$produks->satuan->satuan_produk ?? ""}}"value="{{$produks->id}}" @if ($produks->id == produk_id)seleceted @endif>{{$produks->nama_produk}}</option>@endforeach';                                var myOptions = '<option></option> @foreach ($produk as $produks)<option data-harga="{{$produks->harga_jual}}"data-produk="{{$produks->kode_produk}}"data-satuan="{{$produks->satuan->satuan_produk ?? ''}}"value="{{$produks->id}}">{{$produks->nama_produk}}</option>@endforeach';--}}
                                ...
                                tr.append("</td>");
                                    ...
                                    tr.append("</td>");
                                tr.append('<td><select class="form-control pajak" name="pajak[]"style="font-size: 10px;">'+  '<option value="0">Pilih</option>' + mypajak + '</select>');
                                tr.append("</td>");
