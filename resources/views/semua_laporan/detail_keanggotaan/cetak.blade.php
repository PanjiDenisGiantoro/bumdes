...
<htmlpageheader name="pageHeader">
        <div class="header-border">
            ...
            <h2 class="tajuk" style="padding-top:20px;">Laporan Detail Keanggotaan</h2>
            <br>
...
<div class="struk-border" style="padding-top: 40px;">
    ...
    @foreach($anggotas as $i => $gota)
        <tr>
            <td>{{ $i + 1 }}</td>
            <td>{{ $gota->nama_pemohon ?? ''}}</td>
            <td>{{ $gota->no_mitra ?? ''}}</td>
            <td>{{ $gota->nik ?? ''}}</td>
            <td>{{ date('d/m/Y', strtotime($gota->created_at)) ?? '' }}</td>
            <td>{{ $gota->status_keanggotaans->status_keanggotaan ?? ''}}</td>
       </tr>
    @endforeach
    </table>
</div>
