...
@section('breadcrumb')
    <x-breadcrumb title="{{ __('Tipe Kontak Group') }}">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-options">
                        ...
                        <a style="background-color: blue" href="{{ route('tipe_kontak_group.create') }}">
                            <button class="btn btn-primary">Tambah Tipe Kontak Group</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tbody>
                        <tr>
                            @foreach ($TipeKontakGroup as $i => $kode)
                                <td class="col-lg-none">{{ $TipeKontakGroup->firstItem() + $i }}</td>
                                <td>{{$kode->tipe_kontak_group ?? ''}}</td>
                                <td class="actions">
                                        <a href="{{route('tipe_kontak_group.edit',$kode->id)}}"
                        ...
                        <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id }}">
                            <div class="modal-dialog " role="document">
                              <form action="{{route('tipe_kontak_group.destroy',$kode->id)}}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
