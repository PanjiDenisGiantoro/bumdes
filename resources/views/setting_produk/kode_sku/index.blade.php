            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        ...
                        <a style="background-color: blue" href="{{ route('kode_sku.create') }}">
                            <button class="btn btn-primary">Tambah Kode SKU</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tbody>
                        <tr>
                            @foreach ($KodeSku as $i => $kode)
                                <td class="col-lg-none">{{ $KodeSku->firstItem() + $i }}</td>
                                <td>{{$kode->sku ?? ''}}</td>
                                <td class="actions">
                                        ...
                                        <li><a href="{{ route('kode_sku.edit', $kode->id) }}"
                        ...
                        <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                            <div class="modal-dialog" role="document">
                              <form action="{{route('kode_sku.destroy',$kode->id)}}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
