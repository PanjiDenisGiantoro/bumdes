            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        <a style="background-color: blue" href="{{ route('satuan_produk.create') }}">
                            <button class="btn btn-primary">Tambah Satuan Produk</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tr>
                            ...
                            <th>{{ __('No') }}</th>
                            <th>{{ __('Jenis satuan ') }}</th>
                            <th>{{ __('Tindakan') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            @foreach ($SatuanProduk as $i => $kode)
                                <td class="col-lg-none">{{ $SatuanProduk->firstItem() + $i }}</td>
                                <td>{{$kode->satuan_produk ?? ''}}</td>
                                <td class="actions">
                                        ...
                                        <li><a href="{{ route('satuan_produk.edit', $kode->id) }}"
                        ...
                        <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                          ...
                          <form action="{{route('satuan_produk.destroy',$kode->id)}}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
