            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        <a style="background-color: blue" href="{{ route('bank.create') }}">
                            <button class="btn btn-primary">Tambah Bank</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tbody>
                        <tr>
                            @foreach ($bank as $i => $kode)
                                <td class="col-lg-none">{{$bank->firstItem() + $i }}</td>
                                <td>{{$kode->sandi_bank ?? ''}}</td>
                                <td>{{$kode->nama_bank ?? ''}}</td>
                                 <td class="actions">
                                        ...
                                        <li><a href="{{route('bank.edit',$kode->id)}}"
