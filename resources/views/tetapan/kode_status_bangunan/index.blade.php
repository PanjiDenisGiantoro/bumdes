            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <div class="card-options">
                        ...
                        <a style="background-color: blue" href="{{ route('kode_status_bangunan.create') }}">
                            <button class="btn btn-primary">Tambah Status Bangunan</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tr>
                            ...
                            <th>{{ __('No') }}</th>
                            <th>{{ __('Status Bangunan') }}</th>
                            <th>{{ __('Tindakan') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            @foreach ($KodeStatusBangunan as $i => $kode)
                                <td class="col-lg-none">{{ $KodeStatusBangunan->firstItem() + $i }}</td>
                                <td>{{$kode->status_bangunan ?? ''}}</td>
                                ...
                                <td class="actions">
                                        ...
                                        <li><a href="{{route('kode_status_bangunan.edit',$kode->id)}}"
                        ...
                        <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                            <div class="modal-dialog" role="document">
                                <form action="{{route('kode_status_bangunan.destroy',$kode->id)}}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
