            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        ...
                        <a  style="background-color: blue" href="{{ route('kode_status_keluarga.create') }}">
                            <button class="btn btn-primary">Tambah Status Dalam Keluarga</button>
                        </a>
                    </div>
                </div>
                    ...
                    <table class="table mb-0">
                        <thead>
                            <tr>
                                ...
                                <!-- <th>#</th> -->
                                <th>{{ __('No') }}</th>
                                <th>{{ __('Status Dalam Keluarga') }}</th>
                                <th>{{ __('Tindakan') }}</th>
                                <th>&nbsp;</th>
                            </tr>
                        </thead>
                         <tbody>
                            <tr>
                                @foreach ($KodeStatusKeluarga as $i => $kode)
                                    <td class="col-lg-none">{{ $KodeStatusKeluarga->firstItem() + $i }}</td>
                                <td>{{$kode->status_dalam_keluarga ?? ''}}</td>
                                ...
                                <td class="actions">
                                        ...
                                        <li> <a href="{{route('kode_status_keluarga.edit',$kode->id)}}" class="dropdown-item fa fa-pencil">&nbsp;Edit</a></li>
                            ...
                            <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                                <div class="modal-dialog " role="document">
                                    <form action="{{route('kode_status_keluarga.destroy',$kode->id)}}" method="POST" class="d-inline">
                                        @method('delete')
                                        @csrf
