            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        ...
                        <a  style="background-color: blue" href="{{ route('kode_status_pembiayaan.create') }}">
                            <button class="btn btn-primary">Tambah Tipe Pendanaan</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        <thead>
                         ...
                         <tbody>
                            <tr>
                                @foreach ($KodeStatusPembiayaan as $i => $kode)
                                <td class="col-lg-none">{{ $KodeStatusPembiayaan->firstItem() + $i }}</td>
                                <td>{{$kode->status_pembiayaan ?? ''}}</td>
                                ...
                                <td class="actions">
                                        ...
                                        <li> <a href="{{route('kode_status_pembiayaan.edit',$kode->id)}}" class="dropdown-item fa fa-pencil"  >&nbsp;Edit</a></li>
                                            {{--                                    <button type="button" class="btn btn-danger fa fa-trash"  id="deletebiaya" data-toggle="modal" data-id="{{$kode->id}}" data-target="#exampleModal"></button>--}}
                            ...
                            <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                                <div class="modal-dialog" role="document">
                                    <form action="{{route('kode_status_pembiayaan.destroy',$kode->id)}}" method="POST" class="d-inline">
                                        @method('delete')
                                        @csrf
