            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        <a style="background-color: blue" href="{{ route('sumber_dana.create') }}">
                            <button class="btn btn-primary">Tambah Sumber Dana</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tbody>
                        <tr>
                            @foreach ($sumber_dana as $i => $kode)
                                <td class="col-lg-none">{{$sumber_dana->firstItem() + $i }}</td>
                                <td>{{$kode->nama_sumber_dana ?? ''}}</td>
                                <td class="actions">
                                        ...
                                        <li><a href="{{route('sumber_dana.edit',$kode->id)}}"
                                    ...
                                    <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                                        <div class="modal-dialog " role="document">
                                            <form action="{{route('sumber_dana.destroy',$kode->id)}}" method="POST" class="d-inline">
                                                @method('delete')
                                                @csrf
