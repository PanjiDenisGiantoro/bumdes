            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        ...
                        <a style="background-color: blue" href="{{ route('sumber_pengembalian.create') }}">
                            <button class="btn btn-primary">Tambah Sumber Pengembalian</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tr>
                            ...
                            <th>{{ __('No') }}</th>
                            <th>{{ __('Sumber Pengembalian') }}</th>
                            <th>{{ __('Tindakan') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            @foreach ($sumber_pengembalian as $i => $kode)
                                <td class="col-lg-none">{{ $sumber_pengembalian->firstItem() + $i }}</td>
                                <td>{{$kode->nama_sumber_pengembalian ?? ''}}</td>
                                <td class="actions">
                                        ...
                                        <li><a href="{{route('sumber_pengembalian.edit',$kode->id)}}"
                        ...
                        <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                            <div class="modal-dialog" role="document">
                                <form action="{{route('sumber_pengembalian.destroy',$kode->id)}}" method="POST" class="d-inline">
                                    @method('delete')
                                    @csrf
