            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        ...
                        <a  style="background-color: blue" href="{{ route('template_email.create') }}">
                            <button class="btn btn-primary">Tambah Template Email</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        <thead>
                         ...
                         <tbody>
                         <tr>
                             @foreach ($templateemail as $i => $kode)
                                 <td class="col-lg-none">{{ $templateemail->firstItem() + $i }}</td>
                                 <td>{{$kode->template_email ?? ''}}</td>
                                 <td>{{$kode->keterangan_email ?? ''}}</td>
                                 <td class="actions">
                                        ...
                                        <li><a href="{{route('template_email.edit',$kode->id)}}"
                         ...
                         <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                             <div class="modal-dialog" role="document">
                                 <form action="{{route('template_email.destroy',$kode->id)}}" method="POST" class="d-inline">
