            ...
            <div class="card">
                ...
                <a href="{{ route('tetapan.index') }}" class="btn btn-primary" style="background-color: red">{{ __('Kembali') }}</a>
                    <div class="card-options">
                        <a style="background-color: blue" href="{{ route('tujuan_pengajuan.create') }}">
                            <button class="btn btn-primary">Tambah Tujuan Pengajuan</button>
                        </a>
                    </div>
                </div>
                <div class="table-responsive">
                    <table class="table mb-0">
                        ...
                        <tr>
                            <th>{{ __('No') }}</th>
                            <th>{{ __('Tujuan Pengajuan') }}</th>
                            <th>{{ __('Tindakan') }}</th>
                        </tr>
                        </thead>
                        <tbody>
                        <tr>
                            @foreach ($tujuan_pengajuan as $i => $kode)
                                <td class="col-lg-none">{{$tujuan_pengajuan->firstItem() + $i }}</td>
                                <td>{{$kode->nama_tujuan_pengajuan ?? ''}}</td>
                                <td class="actions">
                                        ...
                                        <li><a href="{{route('tujuan_pengajuan.edit',$kode->id)}}"
                                    ...
                                    <div class="modal fade zoom" tabindex="-1" id="deleteonModal1{{$kode->id}}">
                                        <div class="modal-dialog " role="document">
                                            <form action="{{route('tujuan_pengajuan.destroy',$kode->id)}}" method="POST" class="d-inline">
                                                @method('delete')
                                                @csrf
